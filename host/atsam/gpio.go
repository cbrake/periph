...
func (p *Pin) wrap(err error) error {
	return fmt.Errorf("atsam-gpio (%s): %v", p, err)
}
...
// RegGPIO allows us to manually register GPIOs for devices like Atmel SAM which use
// non-standard GPIO names.
func (d *driverGPIO) RegGPIO(number int, name string) error {
	p := &Pin{
		number: 0,
		name:   name,
		root:   fmt.Sprintf("/sys/class/gpio/%s/", name),
	}

	Pins[number] = p

	return gpioreg.Register(p)
}
...
func (d *driverGPIO) Init() (bool, error) {
	...
	banks := []string{"PA", "PB", "PC", "PD"}
	gpioNum := 0

	for _, bank := range banks {
		for bankNum := 0; bankNum < 32; bankNum++ {
			if _, ok := Pins[gpioNum]; ok {
				return false, fmt.Errorf("found two pins with number %d", gpioNum)
			}

			name := bank + strconv.Itoa(bankNum)

			p := &Pin{
				number: gpioNum,
				name:   name,
				root:   "/sys/class/gpio/" + name + "/",
			}
			Pins[gpioNum] = p
